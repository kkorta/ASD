...
def jumper(g, s, t):
    ...
    while not pq.empty():
        dist, u = pq.get()
        for v in range(n):
            if g[u][v] != 0 and visited[v] is False:

                for x in range(n):
                    if g[v][x] != 0 and distance[x][1] > max(g[u][v], g[v][x]) + distance[u][0]:
                        distance[x][1] = max(g[u][v], g[v][x]) + distance[u][0]

                distance[v][0] = min(distance[u][0] + g[u][v], distance[u][1] + g[u][v], distance[v][0])
                pq.put((distance[v][0], v))
